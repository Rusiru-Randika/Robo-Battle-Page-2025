...
const TeamCard = ({ member, className, index }) => (
  <div
    className={`text-white ${className} flex bg-slate-500`}
  >
    {/* image and background */}
    <div className="relative">
      <img src={`${fileLocation}${member.photo}`} alt={member.name} className="absolute z-20" />
      <div className="z-10 relative w-[146.29px] h-[263.62px] bg-gradient-to-b from-[#D9D9D9] via-[#62B8EE] to-[#009DFF] rounded-[48px_0px]"></div>
    </div>
    {/* details */}
    <div className="relative p-5">
      {/* position */}
      <div className="relative flex justify-center items-center w-[161px] h-[30px] bg-[#0066FF] rounded-[32px_0px]">
        <h3 className="text-sm font-bold ">{member.role}</h3>
      </div>

      <div className="inline-grid text-sm">

        <h1 className="font-poppins font-normal">{member.name}</h1>
        {member.description && (
